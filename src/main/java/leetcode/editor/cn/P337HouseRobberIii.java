...
public class P337HouseRobberIii {
    ...
    class Solution {
        ...
        private int rob(TreeNode root, boolean robRoot) {
            ...
            if (robRoot) {
                Integer num = robMap.get(root);
                if (num != null) {
                    return num;
                }
            } else {
                Integer num = unRobMap.get(root);
                if (num != null) {
                    return num;
                }
            }
            int num;
            if (robRoot) {
                num = root.val + rob(root.left, false) + rob(root.right, false);
                robMap.put(root, num);
            } else {
                int robLeft = rob(root.left, true);
                int robRight = rob(root.right, true);
                int unRobLeft = rob(root.left, false);
                int unRobRight = rob(root.right, false);
                num = Math.max(robLeft, unRobLeft) + Math.max(robRight, unRobRight);
                unRobMap.put(root, num);
            }
            if (num > max) {
                max = num;
            }
